  ...
  return (
    <DashboardShell role="teacher" showBack>
      ...
      <div className="bg-white rounded-[8px] flex items-center px-[16px] py-[18px]">
        ...
        <div className="ml-[12px] bg-white border border-border rounded-[6px] px-[12px] py-[6px]">
          <span className="text-[16px]">학생 선택</span>
        </div>
        <span className="ml-[12px] text-[24px] font-bold text-text-primary">
          {selectedStudent.name} 보호자 2026년 5월 13일 (수)
        </span>
      </div>

      {/* Chat area */}
      <div className="bg-[#d7e8ff] rounded-[8px] flex flex-1 min-h-[600px] overflow-hidden">
        {/* Student list */}
        <div className="w-[195px] bg-white rounded-l-[8px] p-[12px] flex flex-col gap-[8px] overflow-y-auto">
          <div className="bg-white rounded-[8px] p-[8px] text-[14px] font-bold text-center border-b border-border">
            이용인 목록
          </div>
          {students.map((s) => (
            <button
              ...
              className={`text-left px-[10px] py-[8px] rounded-[8px] text-[14px] font-bold transition-colors ${
                selectedStudent.id === s.id
                  ...
                  : "bg-[#eaeaea] text-text-primary hover:bg-[#ddd]"
              }`}
            >
              {s.name}
            </button>
        ...
        {/* Chat panel */}
        <div className="flex-1 bg-white m-[12px] rounded-[8px] flex flex-col">
          {/* Search */}
          ...
          {/* Chat header */}
          <div className="bg-black text-white px-[16px] py-[8px] text-[16px] font-bold rounded-none">
        ...
        {/* CabiSaem toggle panel (right) */}
        {showCabiSaem && (
          <div className="w-[300px] bg-white m-[12px] ml-0 rounded-[8px] p-[16px] flex flex-col gap-[12px]">
